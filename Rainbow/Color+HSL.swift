...
typealias HSL = (hue: Int, saturation: Float, lightness: Float)

extension Color {
  var hsl: HSL {
    var hsl: (h: Float, s: Float, l: Float) = (0, 0, 0) 
    RGB2HSL(self.red(), self.green(), self.blue(), &(hsl.h), &(hsl.s), &(hsl.l))
    return (Int(hsl.h * 360), hsl.s, hsl.l) 
  }
  
  convenience init(hue: Int, saturation: Float, lightness: Float, alpha: Float = 1) {
    let realHue = Float(hue % 360) / 360.0
    var rgb: (r: Float, g: Float, b: Float) = (0, 0, 0) 
    HSL2RGB(realHue, saturation, lightness,  &(rgb.r), &(rgb.g), &(rgb.b))
